...
基于 Vite + React 构建的 MicroApp 微前端子应用，提供订单和产品管理页面。表格和分页组件统一复用 `shared` 远程模块，数据存储依赖 `shared` 提供的 IndexedDB 本地数据库。

## 项目简介

该应用是 [Vite Microapp React Example](https://github.com/liuzane/vite-microapp-react-example.git) 的微前端子应用之一，独立开发、部署，并通过 MicroApp 框架被主应用加载。主要业务功能：
